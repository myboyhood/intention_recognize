...
//	   calibrationOriginVec3 = calibrationOriginVec3 * 20;
  ...
  */
  ...
  /**
   * Get the skeleton's joints and the users IDs and make them all relative to the Torso躯干 joint
  */
  void getSkeleton()
  {
...
//Publish the joint (name, niteConstruct, ConnectedJoint name, niteConstruct, User
        ...
        publishJointTF("right_hand", named_joints["right_hand"], "right_elbow", named_joints["right_elbow"], user.getId());
          ...
          //添加变量让其输出
          geometry_msgs::Point p_LS,p_LE,p_LH,p_LP;
          //LEFT_HAND
          p_LH.x=user.getSkeleton().getJoint(nite::JOINT_LEFT_HAND).getPosition().x;
          p_LH.y=user.getSkeleton().getJoint(nite::JOINT_LEFT_HAND).getPosition().y;
          p_LH.z=user.getSkeleton().getJoint(nite::JOINT_LEFT_HAND).getPosition().z;
          //LEFT_ELBOW
          p_LE.x=user.getSkeleton().getJoint(nite::JOINT_LEFT_ELBOW).getPosition().x;
          p_LE.y=user.getSkeleton().getJoint(nite::JOINT_LEFT_ELBOW).getPosition().y;
          p_LE.z=user.getSkeleton().getJoint(nite::JOINT_LEFT_ELBOW).getPosition().z;
          //LEFT_SHOULDER
          p_LS.x=user.getSkeleton().getJoint(nite::JOINT_LEFT_SHOULDER).getPosition().x;
          p_LS.y=user.getSkeleton().getJoint(nite::JOINT_LEFT_SHOULDER).getPosition().y;
          p_LS.z=user.getSkeleton().getJoint(nite::JOINT_LEFT_SHOULDER).getPosition().z;
          //LEFT_HIP
          p_LP.x=user.getSkeleton().getJoint(nite::JOINT_LEFT_HIP).getPosition().x;
          p_LP.y=user.getSkeleton().getJoint(nite::JOINT_LEFT_HIP).getPosition().y;
          p_LP.z=user.getSkeleton().getJoint(nite::JOINT_LEFT_HIP).getPosition().z;
...
//publishes the funny normal vector from the users chest
  ...
  //publish
  ros::Publisher pub1 = nh_.advertise<geometry_msgs::Point>("pose1",1000);
  ros::Publisher pub2 = nh_.advertise<geometry_msgs::Point>("pose2",1000);
  ros::Publisher pub3 = nh_.advertise<geometry_msgs::Point>("pose3",1000);
  ros::Publisher pub4 = nh_.advertise<geometry_msgs::Point>("pose4",1000);
